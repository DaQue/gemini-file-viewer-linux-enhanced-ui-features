    ...
    egui::Frame::group(ui.style()).show(ui, |ui| {
        ui.horizontal_wrapped(|ui| {
            if let Some(cur) = &app.current_path
                && let Some(name) = cur.file_name().and_then(|s| s.to_str())
            {
            ...
            if matches!(app.content, Some(crate::app::Content::Text(_))) {
                ...
                if enter && app.search_count > 0 {
                    if shift {
                        if app.search_current == 0 {
                            app.search_current = app.search_count.saturating_sub(1);
                        } else {
            ...
            if let Some(cur) = app.current_path.clone() {
                ...
                match app.content {
                    Some(crate::app::Content::Image(_)) => {
                        if ui
                            .small_button(RichText::new("⬅️").size(10.0))
                ...
                && !app.search_query.is_empty()
            {
                ...
                if app.search_count > 0 {
                    ui.add_space(8.0);
                    ui.label(
                        RichText::new(format!("{}/{}", app.search_current + 1, app.search_count))
                            .strong(),
                    );
                }
            }
