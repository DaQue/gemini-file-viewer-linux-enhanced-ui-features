    ...
    egui::Window::new("Recent Files")
        ...
        .show(ctx, |ui| {
            ...
            if ui
                .add_enabled(
                    can_reopen,
                    egui::Button::new(RichText::new("⟳ Reopen Last Session").strong()),
                )
                .clicked()
            {
                ...
                for (idx, p) in app.session_paths.clone().into_iter().enumerate() {
                    ...
                    if idx == active_idx {
                        *file_to_load = Some(p.clone());
                        ...
                        && let Ok((text, lossy, lines)) = crate::io::load_text(&p)
                        && !app.open_text_tabs.iter().any(|t| t.path == p)
                    {
                        app.open_text_tabs.push(crate::app::TextTab {
                            path: p.clone(),
                            text,
                            is_lossy: lossy,
                            line_count: lines,
...
pub(crate) fn global_search_window(ctx: &egui::Context, app: &mut crate::app::FileViewerApp) {
    if !app.show_global_search_window {
        return;
    }
                    ...
                    if app.global_regex {
                        ...
                        ww.widget_info(|| {
                            egui::WidgetInfo::labeled(
                                ...
                                false,
                                "Whole word (disabled in regex)",
                            )
                        });
                    }
                    ui.checkbox(&mut app.global_regex, "Regex");
                });
                if let Some(err) = &app.global_error {
                    ...
                    .show(ui, |ui| {
                        for (idx, res) in app.global_results.clone().into_iter().enumerate() {
                                ...
                                .clicked()
                            {
                                ...
                                {
                                    app.search_count = crate::search::recompute_count(
                                        &app.search_query,
                                        &tab.text,
                                    );
                                }
                                app.search_current = res
                                    .match_index_in_tab
                                    .min(app.search_count.saturating_sub(1));
                                app.scroll_target_line = Some(res.line_index);
                                app.show_global_search_window = false;
                            }
                            if idx < app.global_results.len().saturating_sub(1) {
                                ui.separator();
                            }
                        }
...
pub(crate) fn settings_window(ctx: &egui::Context, app: &mut crate::app::FileViewerApp) {
    ...
    egui::Window::new("Settings")
        ...
        .show(ctx, |ui| {
            ...
            ui.checkbox(
                &mut app.show_line_numbers,
                RichText::new("📊 Line Numbers").strong(),
            );
            ui.checkbox(
                &mut app.use_syntect,
                RichText::new("🎨 Syntect Highlighting").strong(),
            );
            ui.add_space(12.0);
            ui.separator();
            ui.add_space(8.0);
