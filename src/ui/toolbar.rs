...
pub(crate) fn toolbar(
    ...
    let open_clicked = ui
        .add_enabled(!app.file_open_in_flight, open_button)
        .clicked();
    if open_clicked {
    ...
    if ui
        .add_enabled(!app.file_open_in_flight, recent_button)
        .clicked()
    {
    ...
    if ui
        .add_enabled(can_reopen, reopen_button)
        .on_hover_text("Open last session once")
        .clicked()
    {
    ...
    if ui
        .add(
            egui::Button::new(RichText::new("⚙️ Settings").strong())
    ...
    if ui
        .add(
            egui::Button::new(RichText::new("ℹ️ About").strong())
                .fill(egui::Color32::from_rgb(107, 114, 128)),
        )
        .clicked()
    {
    ...
    if matches!(app.content, Some(crate::app::Content::Image(_))) {
        ...
        ui.horizontal(|ui| {
            ui.checkbox(
                &mut app.image_fit,
                RichText::new("📐 Fit to Window").strong(),
            );
            if app.image_fit != prev_fit {
                crate::settings::save_settings_to_disk(app);
            }
        });
        ui.add_space(8.0);
