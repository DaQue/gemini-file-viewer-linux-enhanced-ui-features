            ...
            if let Some(path) = &app.current_path {
                ...
                if let Ok(metadata) = fs::metadata(path) {
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        ui.label(
                            RichText::new(format!("({:.1} KB)", metadata.len() as f64 / 1024.0))
                                .weak(),
                        );
                    });
                }
...
pub(crate) fn status_extra(ui: &mut egui::Ui, app: &mut crate::app::FileViewerApp) {
    egui::Frame::group(ui.style()).show(ui, |ui| {
        ui.horizontal(|ui| match &app.content {
            Some(crate::app::Content::Image(texture)) => {
                ...
                ui.colored_label(
                    ...
                    RichText::new(format!("💾 ~{:.1} MB", est as f64 / (1024.0 * 1024.0))),
                );
                if app.image_fit {
                    ui.add_space(12.0);
                    ui.colored_label(
            ...
            Some(crate::app::Content::Text(_)) => {
                ...
                ui.colored_label(
                    egui::Color32::from_rgb(245, 158, 11),
                    RichText::new(format!("🔍 {:.0}%", app.text_zoom * 100.0)),
                );
                if app.text_is_big {
                    ui.add_space(12.0);
                    ui.colored_label(
                        egui::Color32::from_rgb(239, 68, 68),
                        RichText::new("⚠️ Large file: reduced features"),
                    );
                }
