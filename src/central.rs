    ...
    if let Some(content) = &app.content {
        match content {
            crate::app::Content::Text(text) => {
                ...
                frame.show(ui, |ui| {
                    ui.style_mut().wrap_mode = Some(if app.word_wrap {
                    ...
                    egui::ScrollArea::both()
                        ...
                        .show(ui, |ui| {
                            ...
                            let do_highlight = !app.text_is_big
                            ...
                            if do_line_numbers || do_highlight || !app.search_query.is_empty() {
                                ...
                                let ext = app
                                    ...
                                    .unwrap_or("")
                                    .to_lowercase();
                                let target_line_from_search =
                                    if !app.search_query.is_empty() && app.search_count > 0 {
                                        crate::search::find_target_line(
                                            text,
                                            &app.search_query,
                                            app.search_current,
                                        )
                                ...
                                for (i, line) in text.lines().enumerate() {
                                    ...
                                    if let Some(session) = syntect_session.as_mut() {
                                        ...
                                        crate::highlight::append_highlighted(
                                            &mut line_job,
                                            line,
                                            &mut hctx,
                                        );
                                    ...
                                    if target_line_from_search == Some(i)
                                        || app.scroll_target_line == Some(i)
                                    {
                                        target_rect = Some(resp.rect);
                                    }
                                }
            ...
            crate::app::Content::Image(texture) => {
                ...
                egui::ScrollArea::both().show(ui, |ui| {
                    ui.centered_and_justified(|ui| {
                        ...
                        if app.image_fit {
                            let sx = if size[0] > 0 {
                                viewport.x / size[0] as f32
                            } else {
                                1.0
                            };
                            let sy = if size[1] > 0 {
                                viewport.y / size[1] as f32
                            } else {
                                1.0
                            };
                            let fit = sx.min(sy);
                        ...
                        let desired = egui::vec2(
                            size[0] as f32 * effective_zoom,
                            size[1] as f32 * effective_zoom,
                        );
