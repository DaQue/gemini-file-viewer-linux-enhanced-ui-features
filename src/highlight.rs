...
fn append_rust_line(job: &mut LayoutJob, line: &str, ctx: &mut HighlightContext) {
    let mut i = 0usize;
...
pub(crate) fn token_highlight(job: &mut LayoutJob, text: &str, ctx: &mut HighlightContext) {
    if !ctx.do_syntax {
...
pub(crate) fn append_highlighted(job: &mut LayoutJob, line: &str, ctx: &mut HighlightContext) {
    if ctx.do_syntax {
        if ctx.ext == "rs" {
            append_rust_line(job, line, ctx);
            return;
        }
        if let Some(prefix) = line_comment_prefix(ctx.ext)
