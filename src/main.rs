...
fn build_app_icon() -> egui::IconData {
    ...
    // Separator along diagonal (2px band)
    for y in 0..size {
        for x in 0..size {
    ...
    fill_rect(&mut rgba, 20, 40, 8, 64, code1); // [ left vert
    fill_rect(&mut rgba, 20, 40, 28, 8, code1); // [ top
    fill_rect(&mut rgba, 20, 96, 28, 8, code1); // [ bottom
    fill_rect(&mut rgba, 140, 40, 8, 64, code1); // ] right vert
    fill_rect(&mut rgba, 120, 40, 28, 8, code1); // ] top
